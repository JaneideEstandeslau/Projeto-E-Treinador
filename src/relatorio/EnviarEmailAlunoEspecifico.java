...
public class EnviarEmailAlunoEspecifico extends ClasseParaCriarJanela implements ActionListener{

	public JTextField TFRemetenteEmail;
	public JTextField TFNomeRemetente;
	public JPasswordField TFSenhaRemetente;
	public JFormattedTextField TfDestinatario;
	...
	public EnviarEmailAlunoEspecifico() {
	...
	private void adicionarButon() {
		...
		botapAdicionar.setBounds(550, 192, 90, 30);
	...
	private void adicionarJTextField(JDialog janela) {
		...
		try {
			...
			TfDestinatario = new JFormattedTextField(mascaraDeCpf);
			TfDestinatario.setBounds(390, 125, 250, 23);
		...
		EnderecoAnexo.setBounds(390, 195, 150, 23);
	...
	private void adicionarLabel() {
		...
		this.criarLabol("Destinatário:", 390, 100, 180, 30);
		this.criarLabol("Anexo:", 390, 170, 90, 30);
	...
	public void EmailAnexo() {
		...
		if (Anexo == null) {
			...
			try {
				...
				v.stringNula(TfDestinatario.getText());
				v.stringNula(TFAssunto.getText());

				Aluno aluno = procurar(TfDestinatario.getText());
...

		{
			try {
				...
				mailMessage.setRecipients(Message.RecipientType.TO, InternetAddress.parse(aluno.getEmail()));
	...
	private Aluno procurar(String cpf) {
